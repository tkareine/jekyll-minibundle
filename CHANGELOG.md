...
* Change `ministamp` argument syntax to take in YAML input either as a
  String or Hash. If argument is of Hash type, render values with
  Liquid-like templating support, allowing taking in Liquid
  variables. This change is backwards compatible.

* Add support for specifying destination base url, helping to use the
  plugin with CDN services.
